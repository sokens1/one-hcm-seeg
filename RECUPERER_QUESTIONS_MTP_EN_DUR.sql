-- Script pour récupérer les vraies questions MTP codées en dur et les insérer dans la base
-- Ce script met à jour les offres qui ont des questions vides en base
...
WHERE title = 'Directeur Qualité, Hygiène, Sécurité & Environnement';
...
WHERE title = 'Directeur Juridique, Communication & RSE';
...
WHERE title = 'Chef de Département Electricité';
...
WHERE title = 'Directeur Exploitation Electricité';
...
WHERE title = 'Directeur Technique Electricité';
...
WHERE title = 'Chef de Département Eau';
...
WHERE title = 'Directeur Exploitation Eau';
...
WHERE title = 'Directeur Technique Eau';
...
WHERE title = 'Chef de Département Support';
...
WHERE title = 'Directeur du Capital Humain';
...
WHERE title = 'Directeur Commercial et Recouvrement';
...
WHERE title = 'Directeur Finances et Comptabilité';
...
WHERE title = 'Directeur Moyens Généraux';
...
WHERE title = 'Coordonnateur des Régions';
...
SELECT 
  ...
  status_offerts,
  CASE 
    WHEN mtp_questions_metier IS NULL THEN 'Questions Métier: NULL'
    WHEN array_length(mtp_questions_metier, 1) = 0 THEN 'Questions Métier: VIDE'
    ELSE 'Questions Métier: ' || array_length(mtp_questions_metier, 1) || ' questions'
  END as metier_status,
  CASE 
    WHEN mtp_questions_talent IS NULL THEN 'Questions Talent: NULL'
    WHEN array_length(mtp_questions_talent, 1) = 0 THEN 'Questions Talent: VIDE'
    ELSE 'Questions Talent: ' || array_length(mtp_questions_talent, 1) || ' questions'
  END as talent_status,
  CASE 
    WHEN mtp_questions_paradigme IS NULL THEN 'Questions Paradigme: NULL'
    WHEN array_length(mtp_questions_paradigme, 1) = 0 THEN 'Questions Paradigme: VIDE'
    ELSE 'Questions Paradigme: ' || array_length(mtp_questions_paradigme, 1) || ' questions'
