-- ============================================================================
-- GÉNÉRER LES INSERT COMPLETS AVEC TOUS LES CHAMPS
-- ============================================================================
-- Exécutez cette requête dans votre BASE SOURCE
-- Elle génère les INSERT avec TOUS les champs de la table job_offers
-- ============================================================================
...
SELECT 
    ...
    CASE WHEN requirements IS NULL THEN 'NULL' ELSE quote_literal(REPLACE(requirements, '''', '''''')) END || ', ' ||
    CASE WHEN benefits IS NULL THEN 'NULL' ELSE quote_literal(REPLACE(benefits, '''', '''''')) END || ', ' ||
    ...
    CASE WHEN reporting_line IS NULL THEN 'NULL' ELSE quote_literal(REPLACE(reporting_line, '''', '''''')) END || ', ' ||
    CASE WHEN job_grade IS NULL THEN 'NULL' ELSE quote_literal(job_grade) END || ', ' ||
    CASE WHEN salary_note IS NULL THEN 'NULL' ELSE quote_literal(REPLACE(salary_note, '''', '''''')) END || ', ' ||
    CASE WHEN start_date IS NULL THEN 'NULL' ELSE quote_literal(start_date::text) || '::date' END || ', ' ||
    CASE WHEN responsibilities IS NULL THEN 'NULL' ELSE quote_literal(REPLACE(responsibilities, '''', '''''')) END || ', ' ||
    'CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);' as INSERT_COMPLET
