        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {/* Coverage Overview and Gender Distribution - Side by Side */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <Card>
                  <CardHeader className="pb-3">
                    <div className="flex items-center gap-2">
                      <Target className="h-5 w-5 text-primary" />
                      <CardTitle className="text-base sm:text-lg">Couverture globale des postes</CardTitle>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      Vue d'ensemble de la couverture de tous les postes
                    </p>
                  </CardHeader>
                  <CardContent>
                    <div className="h-64 sm:h-80">
                        ...
                        <RechartsPieChart>
                          <Pie
                            data={[
                              { 
                                name: 'Excellent', 
                                value: jobCoverage.filter(j => j.coverage_status === 'excellent').length,
                                fill: '#10b981' 
                              },
                              { 
                                name: 'Bon', 
                                value: jobCoverage.filter(j => j.coverage_status === 'good').length,
                                fill: '#3b82f6' 
                              },
                              { 
                                name: 'Modéré', 
                                value: jobCoverage.filter(j => j.coverage_status === 'moderate').length,
                                fill: '#f59e0b' 
                              },
                              { 
                                name: 'Faible', 
                                value: jobCoverage.filter(j => j.coverage_status === 'low').length,
                                fill: '#ef4444' 
                              }
                            ]}
                            cx="50%"
                          ...
                          >
                            {[
                              { name: 'Excellent', value: jobCoverage.filter(j => j.coverage_status === 'excellent').length, fill: '#10b981' },
                              { name: 'Bon', value: jobCoverage.filter(j => j.coverage_status === 'good').length, fill: '#3b82f6' },
                              { name: 'Modéré', value: jobCoverage.filter(j => j.coverage_status === 'moderate').length, fill: '#f59e0b' },
                              { name: 'Faible', value: jobCoverage.filter(j => j.coverage_status === 'low').length, fill: '#ef4444' }
                            ].map((entry, index) => (
                          ...
                          <Legend 
                            ...
                            formatter={(value, entry) => (
                              <span style={{ color: entry.color, fontSize: '14px' }}>
                                {value} ({entry.payload.value} postes)
                              </span>
                            )}
                          />
                        </RechartsPieChart>
